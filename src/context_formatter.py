...
def is_cli_environment() -> bool:
    ...
    return (
        os.environ.get("CLAUDE_CODE_CLI") == "true"
        or os.environ.get("TERM_PROGRAM") == "claude-code"
        or any("claude" in arg for arg in sys.argv if arg)
        or not os.isatty(1)
    )  # Check for non-TTY contexts
...
def convert_markdown_to_ansi(text: str, options: Dict[str, Any] = {}) -> str:
    ...
    processed = re.sub(
        ...
        lambda m: (
            m.group(1)
            if strip_only
            else f"{COLORS['BRIGHT']}{COLORS['CYAN']}{m.group(1)}{COLORS['RESET']}"
        ),
        processed,
        flags=re.MULTILINE,
    )
    ...
    processed = re.sub(
        ...
        lambda m: (
            m.group(1)
            if strip_only
            else f"{COLORS['BRIGHT']}{m.group(1)}{COLORS['RESET']}"
        ),
        processed,
        flags=re.MULTILINE,
    )
    ...
    processed = re.sub(
        ...
        lambda m: (
            m.group(1)
            if strip_only
            else f"{COLORS['DIM']}{m.group(1)}{COLORS['RESET']}"
        ),
        processed,
    )
    processed = re.sub(
    ...
    processed = re.sub(
        ...
        lambda m: (
            m.group(1)
            if strip_only
            else f"{COLORS['GRAY']}{m.group(1)}{COLORS['RESET']}"
        ),
        processed,
    )
    ...
    processed = re.sub(
        ...
        lambda m: (
            m.group(1)
            if strip_only
            else f"  {COLORS['CYAN']}•{COLORS['RESET']} {m.group(1)}"
        ),
        processed,
        flags=re.MULTILINE,
    )
    ...
    processed = re.sub(
        ...
        lambda m: (
            m.group(1)
            if strip_only
            else f"{COLORS['CYAN']}{m.group(1)}{COLORS['RESET']}"
        ),
        processed,
    )
    ...
    processed = re.sub(
        r"^[-*_]{3,}$",
        lambda m: "" if strip_only else f"{COLORS['DIM']}{'─' * 40}{COLORS['RESET']}",
        processed,
        flags=re.MULTILINE,
    )
...
def format_memories_for_cli(
    ...
    if include_project_summary and project_context:
        lines.append(
            f"{COLORS['BRIGHT']}{COLORS['CYAN']}📚 Memory Context{COLORS['RESET']}"
        )
        lines.append(f"{COLORS['DIM']}{'─' * 60}{COLORS['RESET']}")
        lines.append(
            f"{COLORS['BRIGHT']}Project:{COLORS['RESET']} {project_context.get('name', 'Unknown')}"
        )

        if project_context.get("language"):
            lines.append(
                f"{COLORS['BRIGHT']}Language:{COLORS['RESET']} {project_context['language']}"
            )
        ...
        if storage_info:
            ...
            color = (
                COLORS["GREEN"] if storage_info.get("connected") else COLORS["YELLOW"]
            )
            lines.append(
                f"{COLORS['BRIGHT']}Storage:{COLORS['RESET']} {color}{storage_type}{COLORS['RESET']}"
            )

        lines.append("")

    # Group memories by category
    categories = {"recent": [], "important": [], "relevant": []}
    ...
    category_configs = [
        (
            ...
            "⭐ Important Context",
            COLORS["YELLOW"],
            max_content_length_categorized,
        ),
        (
            "relevant",
    ...
    lines.append(f"{COLORS['DIM']}{'─' * 60}{COLORS['RESET']}")
    lines.append(
        f"{COLORS['DIM']}💭 {len(memories)} memories loaded • Use /memory-store to save context{COLORS['RESET']}"
    )
...
def format_memories_for_markdown(
    memories: List[Dict[str, Any]],
    project_context: Dict[str, Any] = {},
    options: Dict[str, Any] = {},
) -> str:
