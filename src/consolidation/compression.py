...
class SemanticCompressionEngine(ConsolidationBase):
    ...
    async def process(
        self, clusters: List[MemoryCluster], memories: List[Memory], **kwargs
    ...
    async def _compress_cluster(
        self, cluster: MemoryCluster, memories: List[Memory]
        ...
        key_concepts = await self._extract_key_concepts(
            memories, cluster.theme_keywords
        )
    ...
    async def _extract_key_concepts(
        self, memories: List[Memory], theme_keywords: List[str]
    ) -> List[str]:
    ...
    async def _generate_thematic_summary(
        self, memories: List[Memory], key_concepts: List[str]
    ) -> str:
        ...
        remaining_length = (
            self.max_summary_length - used_length - 100
        )  # Reserve space for conclusion
    ...
    def _aggregate_metadata(self, memories: List[Memory]) -> Dict[str, Any]:
        ...
        aggregated = {"source_memory_hashes": [m.content_hash for m in memories]}
    ...
    async def estimate_compression_benefit(
        self, clusters: List[MemoryCluster], memories: List[Memory]
    ) -> Dict[str, Any]:
