...
class ExponentialDecayCalculator(ConsolidationBase):
    ...
    async def process(self, memories: List[Memory], **kwargs) -> List[RelevanceScore]:
        ...
        memory_connections = kwargs.get(
            "connections", {}
        )  # hash -> connection_count mapping
        access_patterns = kwargs.get(
            "access_patterns", {}
        )  # hash -> last_accessed mapping
    ...
    async def _calculate_memory_relevance(
        ...
        access_boost = self._calculate_access_boost(
            memory, access_patterns, current_time
        )
        ...
        if self._is_protected_memory(memory):
            total_score = max(
                total_score, 0.5
            )  # Minimum 50% relevance for protected memories
    ...
    def _get_base_importance(self, memory: Memory) -> float:
        ...
        if "importance_score" in memory.metadata:
            try:
                ...
                self.logger.warning(
                    f"Invalid importance_score in memory {memory.content_hash}"
                )
    ...
    async def get_low_relevance_memories(
        self, scores: List[RelevanceScore], threshold: float = 0.1
    ) -> List[RelevanceScore]:
