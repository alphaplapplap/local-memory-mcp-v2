...
class DreamInspiredConsolidator:
    ...
    async def consolidate(self, time_horizon: str, **kwargs) -> ConsolidationReport:
        ...
        try:
            ...
            self.logger.info(
                f"Processing {len(memories)} memories for {time_horizon} consolidation"
            )
            ...
            self.logger.debug(
                f"Relevance scoring took {time.time() - performance_start:.2f}s"
            )

            # 3. Cluster by semantic similarity (if enabled and appropriate)
            clusters = []
            if self.config.clustering_enabled and time_horizon in [
                "weekly",
                "monthly",
                "quarterly",
            ]:
                ...
                self.logger.debug(
                    f"Clustering took {time.time() - performance_start:.2f}s, created {len(clusters)} clusters"
                )

            # 4. Run creative associations (if enabled and appropriate)
            associations = []
            if self.config.associations_enabled and time_horizon in [
                "weekly",
                "monthly",
            ]:
                performance_start = time.time()
                ...
                self.logger.debug(
                    f"Association discovery took {time.time() - performance_start:.2f}s, found {len(associations)} associations"
                )
            ...
            if self.config.compression_enabled and clusters:
                ...
                self.logger.debug(
                    f"Compression took {time.time() - performance_start:.2f}s, compressed {len(compression_results)} clusters"
                )
            ...
            forgetting_results = []
            if self.config.forgetting_enabled and time_horizon in [
                "monthly",
                "quarterly",
                "yearly",
            ]:
                ...
                self.logger.debug(
                    f"Forgetting took {time.time() - performance_start:.2f}s, processed {len(forgetting_results)} candidates"
                )
        ...
        if time_horizon == "daily":
            ...
            if time_horizon in ["quarterly", "yearly"]:
                ...
                memories = [
                    ...
                    for m in memories
                    if m.created_at
                    and datetime.utcfromtimestamp(m.created_at) < cutoff_date
                ]
    ...
    def _finalize_report(
        ...
        report.performance_metrics = {
            ...
            "memories_per_second": (
                report.memories_processed / duration if duration > 0 else 0
            ),
            "success": success,
        }
