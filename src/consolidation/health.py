...
class ConsolidationHealthMonitor:
    ...
    async def _check_component_health(self, component: str) -> Dict[str, Any]:
        ...
        now = datetime.now()
        if (
            component in self.component_health_cache
            and component in self.last_health_check
            and now - self.last_health_check[component] < self.cache_ttl
        ):
            ...
            "metrics": {
                "recent_calculations": len(
                    [
                        ...
                        and h.get("timestamp", datetime.min)
    ...
    async def _check_forgetting_engine_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                "recent_archival_operations": len(
                    [
                        h
                        for h in self.performance_history
    ...
    async def _generate_health_recommendations(self) -> List[str]:
        ...
        if recent_errors > 10:
            recommendations.append(
                "High error rate detected. Consider reviewing consolidation configuration."
            )
        ...
        if "average_duration_seconds" in self.metrics:
            ...
            if duration > 300:
                recommendations.append(
                    "Consolidation operations are taking longer than expected. Consider optimizing memory processing."
                )
        ...
        if "storage_response_time" in self.metrics:
            ...
            if response_time > 5.0:
                recommendations.append(
                    "Storage backend response time is elevated. Check database performance."
                )

        return recommendations

    def record_consolidation_performance(
        self,
        time_horizon: str,
        duration: float,
    ...
    def _update_performance_metrics(self):
        ...
        recent_runs = [
            r for r in self.performance_history if r["timestamp"] > recent_cutoff
        ]
        ...
        avg_duration = sum(r["duration_seconds"] for r in recent_runs) / len(
            recent_runs
        )
        ...
        self.metrics["memory_processing_rate"] = HealthMetric(
            ...
            status=self._get_status_for_metric(
                "memory_processing_rate", processing_rate
            ),
        ...
        recent_errors = [
            e for e in self.error_history if e["timestamp"] > recent_error_cutoff
        ]
    ...
    def _create_alert(self, component: str, severity: HealthStatus, message: str):
        ...
        alert_id = (
            f"{component}_{severity.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )

        alert = HealthAlert(
            alert_id=alert_id, component=component, severity=severity, message=message
        )
        ...
        self.logger.warning(
            f"Health alert [{severity.value}] for {component}: {message}"
        )
    ...
    async def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_alerts": len(
                [a for a in health["alerts"] if not a.get("resolved", False)]
            ),
            "critical_alerts": len(
                [
                    a
