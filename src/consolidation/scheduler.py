...
class ConsolidationScheduler:
    ...
    def __init__(
        ...
        self.execution_stats = {"total_jobs": 0, "successful_jobs": 0, "failed_jobs": 0}
        ...
        if APSCHEDULER_AVAILABLE and enabled:
            ...
            if not APSCHEDULER_AVAILABLE:
                self.logger.warning(
                    "APScheduler not available - consolidation scheduling disabled"
                )
            elif not enabled:
        ...
        for horizon in time_horizons:
            ...
            try:
                ...
                if trigger:
                    ...
                    self.logger.info(
                        f"Scheduled {horizon} consolidation: {schedule_spec}"
                    )
    ...
    def _create_trigger(self, horizon: str, schedule_spec: str):
        ...
        try:
            if horizon == "daily":
                ...
                if len(day_time) != 2:
                    raise ValueError(
                        f"Invalid monthly schedule format: {schedule_spec}"
                    )
                ...
                if len(parts) != 2:
                    raise ValueError(
                        f"Invalid quarterly schedule format: {schedule_spec}"
                    )
            ...
            self.logger.error(
                f"Error creating trigger for {horizon} with spec '{schedule_spec}': {e}"
            )
            return None
    ...
    async def _run_consolidation_job(self, time_horizon: str):
        ...
        try:
            ...
            # Log success
            duration = (
                job_record["end_time"] - job_record["start_time"]
            ).total_seconds()
            self.logger.info(
    ...
    async def trigger_consolidation(
        self, time_horizon: str, immediate: bool = True
    ) -> bool:
    ...
    async def get_scheduler_status(self) -> Dict[str, Any]:
        ...
        if not self.scheduler:
            return {"enabled": False, "reason": "APScheduler not available or disabled"}
        ...
        return {
            ...
            "last_execution_times": {
                horizon: time.isoformat()
                for horizon, time in self.last_execution_times.items()
            },
        ...
        try:
            if time_horizon:
                ...
                if job:
                    ...
                    self.logger.warning(
                        f"No job found for {time_horizon} consolidation"
                    )
            else:
