...
class ControlledForgettingEngine(ConsolidationBase):
    ...
    async def process(
        self, memories: List[Memory], relevance_scores: List[RelevanceScore], **kwargs
    ...
    async def _identify_forgetting_candidates(
        ...
        for memory in memories:
            ...
            if forgetting_reasons:
                ...
                if not (time_horizon in ["quarterly", "yearly"]):
                    ...
                    if not (
                        "expired_temporary" in forgetting_reasons
                        or "potential_duplicate" in forgetting_reasons
                    ):
        ...
        candidates.sort(
            key=lambda c: (c.archive_priority, -c.relevance_score.total_score)
        )
    ...
    def _is_low_quality_content(self, memory: Memory) -> bool:
        ...
        if (
            len(set(content.split())) < len(content.split()) * 0.5
        ):  # Less than 50% unique words
            return True
    ...
    def _appears_to_be_duplicate(
        self, memory: Memory, all_memories: List[Memory]
    ) -> bool:
    ...
    async def _process_forgetting_candidate(
        self, candidate: ForgettingCandidate
    ) -> ForgettingResult:
        ...
        try:
            ...
            if (
                candidate.can_be_deleted
                and "potential_duplicate" in candidate.forgetting_reasons
            ):
            ...
            self.logger.error(
                f"Error processing forgetting candidate {memory.content_hash}: {e}"
            )
    ...
    async def _compress_memory(
        ...
        short_hash = memory.content_hash[:12]
        archive_file = (
            self.compressed_archive / f"original_{timestamp}_{short_hash}.json"
        )
    ...
    async def _create_metadata_entry(
        self, memory: Memory, archive_path: Path, action: str
    ):
        ...
        for archive_dir in [self.daily_archive, self.compressed_archive]:
            for archive_file in archive_dir.glob("*.json"):
                try:
                    ...
                    self.logger.warning(
                        f"Error reading archive file {archive_file}: {e}"
                    )
            ...
            try:
                with open(log_file, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if timestamp:
                            if (
                                not stats["oldest_archive"]
                                or timestamp < stats["oldest_archive"]
                            ):
                                stats["oldest_archive"] = timestamp
                            if (
                                not stats["newest_archive"]
                                or timestamp > stats["newest_archive"]
                            ):
