...
class CreativeAssociationEngine(ConsolidationBase):
    ...
    async def process(
        ...
        self.logger.info(
            f"Discovered {len(associations)} creative associations from {len(pairs)} pairs"
        )
        return associations
    ...
    async def _analyze_association(
        self, mem1: Memory, mem2: Memory, similarity: float
    ) -> AssociationAnalysis:
    ...
    async def _create_association_memory(
        self, analysis: AssociationAnalysis
    ) -> MemoryAssociation:
    ...
    async def filter_high_confidence_associations(
        self, associations: List[MemoryAssociation], min_confidence: float = 0.5
    ) -> List[MemoryAssociation]:
