...
class APIErrorHandler:
    ...
    def init_app(self, app: Flask):
        ...
        app.register_error_handler(
            ServiceUnavailableError, self.handle_service_unavailable_error
        )
        app.register_error_handler(TimeoutError, self.handle_timeout_error)
        app.register_error_handler(
            InsufficientDataError, self.handle_insufficient_data_error
        )
        app.register_error_handler(
            ResourceExhaustedError, self.handle_resource_exhausted_error
        )
    ...
    def handle_memory_system_error(self, error: MemorySystemError) -> Response:
        ...
        return self._create_error_response(
            error=error, status_code=get_http_status_code(error), include_details=True
        )

    def handle_validation_error(self, error: ValidationError) -> Response:
        """Handle ValidationError"""
        return self._create_error_response(
            error=error, status_code=400, include_details=True
        )
    ...
    def handle_service_unavailable_error(
        self, error: ServiceUnavailableError
    ) -> Response:
    ...
    def handle_timeout_error(self, error: TimeoutError) -> Response:
        """Handle TimeoutError"""
        return self._create_error_response(
            error=error, status_code=504, include_details=False
        )

    def handle_insufficient_data_error(self, error: InsufficientDataError) -> Response:
        """Handle InsufficientDataError"""
        return self._create_error_response(
            error=error, status_code=422, include_details=True
        )

    def handle_resource_exhausted_error(
        self, error: ResourceExhaustedError
    ) -> Response:
        """Handle ResourceExhaustedError"""
        return self._create_error_response(
            error=error, status_code=507, include_details=False
        )

    def handle_http_exception(self, error: HTTPException) -> Response:
        """Handle Werkzeug HTTPException"""
        return self._create_error_response(
            error=error, status_code=error.code, include_details=False
        )
    ...
    def _create_error_response(
        ...
        if status_code >= 500:
            logger.error(
                f"Server error {status_code}: {error}",
                extra={
                    "status_code": status_code,
            ...
            logger.warning(
                f"Client error {status_code}: {error}",
                extra={
                    "status_code": status_code,
                    "error_type": type(error).__name__,
                    "request_id": error_response.get("request_id"),
...
def api_endpoint(success_message: str = "Success", include_metadata: bool = False):
            ...
            try:
                ...
                if isinstance(result, tuple):
                    ...
                    return create_success_response(
                        ...
                        metadata=(
                            {"endpoint": func.__name__} if include_metadata else None
                        ),
                    )
                else:
                    return create_success_response(
