...
from error_handlers import RetryConfig, error_context, retry_on_error
...
class DatabaseErrorHandler:
    ...
    @classmethod
    def handle_psycopg2_error(
        cls, error: psycopg2.Error, operation: str = None, table: str = None
    ) -> Exception:
...
def database_operation(operation: str, table: str = None, connection_pool=None):
    ...
    try:
        ...
        logger.debug(
            f"Database operation '{operation}' on '{table}' completed in {duration:.3f}s"
        )
        ...
        logger.error(
            f"Database operation '{operation}' on '{table}' failed after {duration:.3f}s: {e}"
        )
        raise custom_error
        ...
        duration = time.time() - start_time
        logger.error(
            f"Unexpected error in database operation '{operation}' on '{table}' after {duration:.3f}s: {e}"
        )
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt == max_attempts - 1:
                        # Last attempt failed
                        logger.error(
                            f"Database operation '{operation}' on '{table}' failed after {max_attempts} attempts"
                        )
                        raise e
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        try:
            with database_operation(
                "HEALTH_CHECK", connection_pool=self.connection_pool
            ) as cursor:
                ...
                if result and result[0] == 1:
                    ...
                    if self.connection_pool:
                        self.health_metrics["connection_pool_status"] = (
                            self.connection_pool.get_pool_status()
                        )
