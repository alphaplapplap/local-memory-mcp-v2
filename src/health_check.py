...
class MemorySystemHealthChecker(HealthChecker):
    ...
    async def _check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memories = self.memory_api.retrieve_memories(
                "health check", limit=1, domain=test_domain
            )
            ...
            try:
                ...
                logger.warning(f"Failed to cleanup test memory: {e}")
...
class ConsolidationHealthChecker(HealthChecker):
    ...
    async def _check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                not hasattr(self.memory_api, "consolidator")
                or not self.memory_api.consolidator
            ):
                return {
                    "available": False,
            ...
            return {
                "available": True,
                "clustering_working": cluster_result.get("status")
                in ["success", "insufficient_data"],
...
class SystemHealthChecker:
    ...
    def _initialize_checkers(self):
        ...
        # Ollama checker
        if (
            hasattr(self.memory_api, "ollama_embeddings")
            and self.memory_api.ollama_embeddings
        ):
    ...
    def get_overall_status(self) -> HealthStatus:
        ...
        for result in self.last_results:
            if (
                result.name in critical_components
        ...
        all_healthy = all(
            result.status == HealthStatus.HEALTHY for result in self.last_results
        )
        if all_healthy:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "healthy": sum(
                    1 for r in self.last_results if r.status == HealthStatus.HEALTHY
                ),
                "degraded": sum(
                    1 for r in self.last_results if r.status == HealthStatus.DEGRADED
                ),
                "unhealthy": sum(
                    1 for r in self.last_results if r.status == HealthStatus.UNHEALTHY
                ),
            },
        }
    ...
    def is_healthy(self) -> bool:
        ...
        return self.get_overall_status() in [
