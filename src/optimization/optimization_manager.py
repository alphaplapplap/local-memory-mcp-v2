...
class OptimizationManager:
    ...
    def optimize_memory_storage(
        ...
        should_store, quality_score, reason = self.memory_api._assess_content_quality(content)
    ...
    def optimize_memory_retrieval(
        ...
        if self.consolidator.should_consolidate(len(memories)):
            ...
            for action in consolidation_report.get('actions', []):
                if action['type'] == 'merge':
                    merged_ids.update(action['memory_ids'][1:])  # Keep first, remove others
        ...
        if len(memories) > 10:  # Only cluster if we have enough memories
            ...
            if clusters:
                ...
                representatives = self.clusterer.get_cluster_representatives(
                    domain,
                    limit=min(10, token_budget // 100)  # Rough estimate: 100 tokens per rep
                )
                ...
                metadata['cluster_reduction'] = 1 - (len(clustered_memories) / metadata['original_count'])
        ...
        if memories:
            ...
            if cluster_ids:
                context, context_meta = self.progressive_summarizer.get_progressive_context(
                    query=query,
                    domain=domain,
        ...
        metadata['reduction_rate'] = 1 - (len(memories) / metadata['original_count']) if metadata['original_count'] > 0 else 0
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        if stats['memories_processed'] > 0:
            stats['rejection_rate'] = stats['memories_rejected'] / stats['memories_processed']
            stats['consolidation_rate'] = stats['memories_consolidated'] / stats['memories_processed']
        else:
        ...
        stats['avg_token_savings'] = (
            ...
            if stats['memories_processed'] > 0 else 0
        )
    ...
    def run_full_optimization(
        ...
        # Phase 2: Consolidate
        if len(memories) > 10:
            consolidation = self.consolidator.consolidate_memories(memories, dry_run=False)
        ...
        if len(memories) > 5:
            ...
            if clusters:
                representatives = self.clusterer.get_cluster_representatives(domain, limit=10)
                rep_ids = [r['memory_id'] for r in representatives]
        ...
        report['total_reduction'] = 1 - (len(memories) / report['input_count']) if report['input_count'] > 0 else 0
