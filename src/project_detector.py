...
def detect_language(directory: str) -> Dict[str, Any]:
    ...
    try:
        ...
        confidence = min(best_score / 50, 1.0) if best_score > 0 else 0
...
def detect_framework(directory: str) -> Dict[str, Any]:
    ...
    try:
        ...
        config_checks = {
            ...
            "pyproject.toml": lambda path: _check_pyproject_toml(
            ...
            "Dockerfile": lambda path: (tools.append("Docker"), None)[1],
            ".env": lambda path: (tools.append("Environment Config"), None)[1],
        }
        ...
        return {"frameworks": frameworks, "tools": tools, "projectName": project_name}
...
def _check_package_json(
    filepath: str, frameworks: List[str], tools: List[str]
) -> Optional[str]:
...
def detect_project_context(directory: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"{COLORS['BLUE']}📂 Project Detector{COLORS['RESET']} {COLORS['DIM']}→{COLORS['RESET']} Analyzing {COLORS['BRIGHT']}{directory_name}{COLORS['RESET']}"
        )
        ...
        confidence_color = (
            ...
            else COLORS["YELLOW"] if context["confidence"] > 0.6 else COLORS["GRAY"]
        )

        print(
            f"{COLORS['BLUE']}📊 Detection Result{COLORS['RESET']} {COLORS['DIM']}→{COLORS['RESET']} {COLORS['BRIGHT']}{context['name']}{COLORS['RESET']} {COLORS['GRAY']}({context['language']}){COLORS['RESET']} {COLORS['DIM']}•{COLORS['RESET']} {confidence_color}{confidence_percent}%{COLORS['RESET']}"
        )
        ...
        print(
            f"{COLORS['RED']}❌ Project Detector Error{COLORS['RESET']} {COLORS['DIM']}→{COLORS['RESET']} {error}"
        )
