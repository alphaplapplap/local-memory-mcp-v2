...
class Memory:
    ...
    def _sync_timestamps(
        self, created_at=None, created_at_iso=None, updated_at=None, updated_at_iso=None
    ):
        """
        ...
        if created_at is not None and created_at_iso is not None:
            ...
            try:
                ...
                if (
                    time_diff > 1.0 and time_diff < 86400
                ):  # Between 1 second and 24 hours suggests timezone issue
                    logger.info(
                        f"Timezone mismatch detected (diff: {time_diff}s), preferring float timestamp"
                    )
                    ...
                    time_diff >= 86400
                ):  # More than 24 hours difference suggests data corruption
                    logger.warning(
                        f"Large timestamp difference detected ({time_diff}s), using current time"
                    )
                    self.created_at = now
        ...
        if updated_at is not None and updated_at_iso is not None:
            ...
            try:
                ...
                if (
                    ...
                    logger.info(
                        f"Timezone mismatch detected in updated_at (diff: {time_diff}s), preferring float timestamp"
                    )
                    # Use the float timestamp as authoritative and regenerate ISO
                    ...
                    logger.warning(
                        f"Large timestamp difference detected in updated_at ({time_diff}s), using current time"
                    )
                    self.updated_at = now
                ...
                logger.warning(
                    f"Error parsing updated timestamps: {e}, using float timestamp"
                )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timestamp": float(
                self.created_at
            ),  # Changed from int() to preserve precision
    ...
    @classmethod
    def from_dict(
        cls, data: Dict[str, Any], embedding: Optional[List[float]] = None
    ) -> "Memory":
