...
class MemorySystem:
    ...
    def _setup_ollama_embeddings(self, 
                                ollama_url: Optional[str] = None,
                                embedding_model: Optional[str] = None,
                                keep_alive: Optional[str] = None) -> Optional[OllamaEmbeddings]:
        """Setup Ollama embeddings if available."""
        ollama_url = ollama_url or os.getenv("OLLAMA_API_URL", "http://localhost:11434")
        embedding_model = embedding_model or os.getenv("OLLAMA_EMBEDDING_MODEL", "nomic-embed-text:v1.5")
    ...
    def delete(self, memory_id: str, domain: Optional[str] = None) -> bool:
        ...
        raise NotImplementedError("Delete functionality not yet implemented in PostgresMemoryAPI")
    ...
    def get_memory(self, memory_id: str, domain: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.ollama_embeddings:
            stats["embedding_model"] = getattr(self.ollama_embeddings, 'model_name', 'unknown')
...
class AsyncMemorySystem:
    ...
    async def search(self, 
        ...
        return await loop.run_in_executor(
            None,
            self.memory_api.retrieve_memories,
            query,
            limit,
            domain
        )
...
def quick_search(query: str, domain: str = "default", limit: int = 5) -> List[Dict[str, Any]]:
    """Quick function to search memories."""
    memory = MemorySystem()
    return memory.search(query, domain, limit)


def quick_ingest(file_path: Union[str, Path], domain: str = "documents") -> Dict[str, Any]:
...
async def async_quick_search(query: str, domain: str = "default", limit: int = 5) -> List[Dict[str, Any]]:
    """Quick async function to search memories."""
    memory = AsyncMemorySystem()
    return await memory.search(query, domain, limit)


async def async_quick_ingest(file_path: Union[str, Path], domain: str = "documents") -> Dict[str, Any]:
...
if __name__ == "__main__":
    async def async_example():
        ...
        mem1 = await memory.store("User loves async programming", "personal", importance=0.9)
        mem2 = await memory.store("Async operations are faster", "technical", source="learning")
        print(f"Async stored memories: {mem1}, {mem2}")
        
        # Search asynchronously
        results = await memory.search("async programming", limit=2)
        for i, result in enumerate(results, 1):
            print(f"{i}. {result['content'][:40]}... (score: {result.get('score', 0):.3f})")
    ...
    def sync_example():
        ...
        # Store some memories
        print("\n📝 Storing memories...")
        mem1 = memory.store("User prefers Python for data analysis", "personal", importance=0.8)
        mem2 = memory.store("Meeting scheduled for Tuesday at 3pm", "work", source="calendar")
