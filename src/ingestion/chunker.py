...
# Adapted for local-memory-mcp PostgreSQL system
...
class ChunkingStrategy:
    ...
    chunk_overlap: int = (
        100  # Characters to overlap between chunks (optimized from 200)
    )
    respect_sentence_boundaries: bool = True
    respect_paragraph_boundaries: bool = True
    min_chunk_size: int = (
        200  # Minimum characters for a valid chunk (optimized from 100)
    )
...
class TextChunker:
    ...
    def _chunk_by_paragraphs(self, text: str) -> List[str]:
        ...
        for paragraph in paragraphs:
            ...
            if (
                len(current_chunk) + len(paragraph) + 2 > self.strategy.chunk_size
    ...
    def _chunk_by_sentences(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if (
                len(current_chunk) + len(sentence) + 1 > self.strategy.chunk_size
                and len(current_chunk) > 0
            ):
    ...
    def _chunk_by_characters(self, text: str) -> List[str]:
        ...
        while start < len(text):
            ...
            if end < len(text):
                ...
                for i in range(
                    end, max(start + self.strategy.min_chunk_size, end - 100), -1
                ):
                    if text[i].isspace():
                        end = i
                        break
