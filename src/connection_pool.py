...
class DatabaseConnectionPool:
    ...
    def _initialize_pool(self):
        ...
        try:
            ...
            if self._is_test_environment():
                logger.warning(
                    "Running in test environment - connection pool not initialized, "
                    "using direct connections"
                )
                self._pool = None
            else:
                logger.error(f"Failed to initialize connection pool in production: {e}")
                raise
    ...
    def _is_test_environment(self) -> bool:
        ...
        return (
            ...
            or "test" in self.connection_params.get("user", "").lower()
            or "test" in self.connection_params.get("database", "").lower()
        )
...
class ConnectionHealthChecker:
    ...
    def is_database_healthy(self) -> bool:
        ...
        if (
            self.last_check is None or time.time() - self.last_check > 60
        ):  # Check every minute
