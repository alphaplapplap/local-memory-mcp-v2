...
def calculate_tag_relevance(
    memory_tags: List[str] = [], project_context: Dict[str, Any] = {}
) -> float:
    """
    ...
    try:
        ...
        exact_project_match = (
            project_name in memory_tags_lower if project_name else False
        )
        ...
        total_score = min(
            1.0, overlap_score + project_bonus + language_bonus + framework_bonus
        )
...
def calculate_content_quality(memory_content: str = "") -> float:
    ...
    try:
        ...
        has_structure = any(
            marker in memory_content for marker in ["- ", "* ", "1.", "## ", "**"]
        )
...
def calculate_conversation_relevance(
    memory: Dict[str, Any], conversation_analysis: Dict[str, Any]
) -> float:
    """
...
def score_memory_relevance(
    ...
    logger.debug(
        f'[Memory Scorer] Scoring {len(memories)} memories for project: {project_context.get("name", "unknown")}'
    )
    ...
    for memory in memories:
        try:
            ...
            tag_score = calculate_tag_relevance(memory.get("tags", []), project_context)

            quality_score = calculate_content_quality(memory.get("content", ""))

            importance_score = calculate_importance_weight(memory.get("metadata", {}))

            conversation_score = 0.5  # Default
            if include_conversation and conversation_analysis:
                conversation_score = calculate_conversation_relevance(
                    memory, conversation_analysis
                )
    ...
    for i, memory in enumerate(scored_memories[:3]):
        ...
        logger.debug(
            f'  {i+1}. Score: {memory["relevanceScore"]:.3f} - {content_preview}'
        )
