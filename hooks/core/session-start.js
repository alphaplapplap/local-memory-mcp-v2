...
async function queryHealthEndpoint(endpoint, apiKey, options = {}) {
    ...
    return new Promise((resolve) => {
        try {
            ...
            const defaultPort = url.protocol === 'https:' ? 443 : 8000;
 ...
 */
function parseHealthDataToStorageInfo(healthData, config = {}) {
    try {
        ...
        // Build location info
        let location = storage.database_path || storage.location;

        // For PostgreSQL, extract from endpoint URL if available
        if (!location && storage.backend === 'postgresql' && config.endpoint) {
            try {
                const url = new URL(config.endpoint);
                location = `${url.hostname}:${url.port || 5432}`;
            } catch {
...
async function queryMemoryService(endpoint, apiKey, query) {
    return new Promise((resolve, reject) => {
        ...
        // Use the local API endpoint for direct memory search
        const url = new URL('/api/memories/search', endpoint);
        const postData = JSON.stringify({
            query: query.semanticQuery || '',
            limit: query.limit || 10,
            domain: query.domain || 'default',
            time_filter: query.timeFilter || null
        });
 ...
 */
async function checkMemoryHealth(endpoint, apiKey, useDetailed = false, timeout = 3000) {
    try {
        const healthResult = await queryHealthEndpoint(endpoint, apiKey, {
            timeout,
            useDetailed
        });

        if (healthResult.success && healthResult.data) {
            const storageInfo = parseHealthDataToStorageInfo(healthResult.data, { endpoint });
            return storageInfo.health || { accessible: false };
        }
    } catch (error) {
...
async function onSessionStart(context) {
    try {
        ...
        const serverHealthCheck = await checkMemoryHealth(
            config.memoryService?.endpoint || 'http://localhost:8000',
            config.memoryService?.apiKey,
            false,
            1000
        );

        if (verbose && !cleanMode && serverHealthCheck.accessible) {
            console.log(`${CONSOLE_COLORS.GREEN}⚡ Memory Server${CONSOLE_COLORS.RESET} ${CONSOLE_COLORS.DIM}→${CONSOLE_COLORS.RESET} Already running and healthy`);
        }
        ...
        if (showStorageSource && verbose && !cleanMode) {
            ...
            if (healthCheckEnabled) {
                ...
                if (healthResult.success) {
                    storageInfo = parseHealthDataToStorageInfo(healthResult.data, { endpoint: config.memoryService.endpoint });
                ...
                if (sourceDisplayMode === 'detailed') {
                    ...
                    if (storageInfo.location && sourceDisplayMode === 'brief') {
                        console.log(`${CONSOLE_COLORS.CYAN}📍 Path${CONSOLE_COLORS.RESET} ${CONSOLE_COLORS.DIM}→${CONSOLE_COLORS.RESET} ${CONSOLE_COLORS.GRAY}${projectContext.directory}${CONSOLE_COLORS.RESET}`);
                    }
        ...
        if (recentFirstMode) {
            ...
            if (remainingSlotsAfterGit > 0) {
                ...
                if (verbose && showMemoryDetails && !cleanMode) {
                    console.log(`${CONSOLE_COLORS.YELLOW}🕒 Phase 1${CONSOLE_COLORS.RESET} ${CONSOLE_COLORS.DIM}→${CONSOLE_COLORS.RESET} Searching recent memories (${recentTimeWindow}, ${recentQuery.limit || recentSlots} slots)`);
                }
            ...
            if (remainingSlots > 0) {
                ...
                if (verbose && showMemoryDetails && !cleanMode) {
                    console.log(`${CONSOLE_COLORS.YELLOW}🎯 Phase 2${CONSOLE_COLORS.RESET} ${CONSOLE_COLORS.DIM}→${CONSOLE_COLORS.RESET} Searching important tagged memories (${remainingSlots} slots)`);
                }
            ...
            if (stillRemaining > 0 && allMemories.length < 3) {
                ...
                if (verbose && showMemoryDetails && !cleanMode) {
                    console.log(`${CONSOLE_COLORS.YELLOW}🔄 Phase 3${CONSOLE_COLORS.RESET} ${CONSOLE_COLORS.DIM}→${CONSOLE_COLORS.RESET} Fallback general context (${stillRemaining} slots, ${fallbackTimeWindow})`);
                }
