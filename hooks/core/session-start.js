    ...
    try {
        ...
        if (envBackend) {
            switch (envBackend) {
                ...
                case 'postgresql':
                case 'postgres':
                    const pgHost = process.env.POSTGRES_HOST || config.postgres?.host;
                    const pgPath = process.env.POSTGRES_DB_PATH;

                    if (pgHost) {
                        // Remote PostgreSQL
                        const pgPort = process.env.POSTGRES_PORT || config.postgres?.port || '5432';
                        const pgDatabase = process.env.POSTGRES_DB || config.postgres?.database || 'postgres';
                        const ssl = process.env.POSTGRES_SSL === 'true';
                        const protocol = ssl ? 'postgresql+ssl' : 'postgresql';
                        storageInfo = {
                            backend: 'postgresql',
                            type: 'remote',
                            location: `${protocol}://${pgHost}:${pgPort}/${pgDatabase}`,
                            description: 'PostgreSQL + pgvector (Remote Config)',
                            icon: '🌐',
                            health: { status: 'unknown', totalMemories: 0 }
                        };
                    } else {
                        ...
                        storageInfo = {
                            backend: 'postgresql',
                            type: 'local',
                            location: pgPath || 'localhost:5432/postgres',
                            description: 'PostgreSQL + pgvector (Config)',
                            icon: '🐘',
                            health: { status: 'unknown', totalMemories: 0 }
                        };
                    }
